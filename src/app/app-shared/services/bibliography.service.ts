...
export class BibliographyService {
    ...
    constructor(private _http: HttpService,private _sharedService: SharedService,private userService : AccountService, private _config: SharedConfiguration, private _serviceUrlManager: ServiceUrlManager, private _sharedRootComponentValues: SharedRootComponentValues) {
    ...
    CarvingGetAllDocumetnts(page:number, searchSD:SearchSDModel, pageSize = this._config.PageSize): Observable<summarydocumentsResponse> {
        ...
        return this._http.post((this._serviceUrlManager.getServiceUrl(ServicesIDs.CarvingGetAllDocumetnts) + parm), searchSD).pipe(catchError((error: any) => { return this._sharedService.handleError(error) }),
            map(data => data)
            );
    }
    ...
    private GetAllDocumetnts(page:number, searchSD:SearchSDModel, pageSize = this._config.PageSize): Observable<summarydocumentsResponse> {
        const parm = '?page=' + page + '&pageSize=' + pageSize;
        return this._http.post((this._serviceUrlManager.getServiceUrl(ServicesIDs.GetAllDocumetnts) + parm), searchSD).pipe(catchError((error: any) => { return this._sharedService.handleError(error) }),
            map(data => data)
            );
    }
    ...
    getAllCarvingSourceABV(page:number, pageSize = this._config.PageSize): Observable<carvingsourceabvResponse> {
        const parm = '?page=' + page + '&pageSize=' + pageSize;
        return this._http.get((this._serviceUrlManager.getServiceUrl(ServicesIDs.GetAllCarvingSourceABV) + parm)).pipe(catchError((error: any) => { return this._sharedService.handleError(error) }),
            map(data => data)
            );
    }
    getAllCarvingSource(page:number, pageSize = this._config.PageSize): Observable<carvingsourceResponse> {
        const parm = '?page=' + page + '&pageSize=' + pageSize;
        return this._http.get((this._serviceUrlManager.getServiceUrl(ServicesIDs.GetAllCarvingSource) + parm)).pipe(catchError((error: any) => { return this._sharedService.handleError(error) }),
            map(data => data)
            );
    }
    GetSummaryDocumentDates(): Observable<number[]> {
        return this._http.get(this._serviceUrlManager.getServiceUrl(ServicesIDs.GetSummaryDocumentDates)).pipe(catchError((error: any) => { return this._sharedService.handleError(error) }),
            map(data => data)
            );
    }
