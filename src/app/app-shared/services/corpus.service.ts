...
export class CorpusService {
    ...
    constructor(private _http: HttpService, private _serviceUrlManager: ServiceUrlManager,
    ...
    public getcorpus(searchSD:any): Observable<corpussearchResponse> {
        return this._http.post((this._serviceUrlManager.getServiceUrl(ServicesIDs.SearchInCorpus)), searchSD).pipe(
            map(data => data));
    }


    public getNextSequence(sequenceId:number): Observable<summarycorpusmodel> {
        return this._http.get((this._serviceUrlManager.getServiceUrl(ServicesIDs.GetNextSequence)+'?sequenceId='+sequenceId)).pipe(
            map(data => data));
    }


    public getPrevSequence(sequenceId:number): Observable<summarycorpusmodel> {
        return this._http.get((this._serviceUrlManager.getServiceUrl(ServicesIDs.GetPrevSequence)+'?sequenceId='+sequenceId)).pipe(
            map(data => data));
    }
