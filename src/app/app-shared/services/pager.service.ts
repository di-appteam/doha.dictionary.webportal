...
export class PagerService {
    ...
    getRootPager(totalItems: number, currentPage: number = 1, pageSize: number = this._config.PageSize) {
        ...
        const pages = this.RootRange(startPage, endPage, 1);
    ...
    getPager(totalItems: number, currentPage: number = 1, pageSize: number = this._config.PageSize) {
        // calculate total pages
        const totalPages = Math.ceil(totalItems / pageSize);
    ...
    RootRange = function (start, stop, step) {
        ...
        const range = Array(stop);

        for (let idx = 0; idx < stop; idx++ , start += step) {
            range[idx] = start;
        }
